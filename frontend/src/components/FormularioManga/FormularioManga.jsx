...
function FormularioManga(props) {
  ...
  const handleBlur = (key) => {
    switch(key){
      case 'textarea': return setIsTextAreaTouched(true);
      case 'date-input': return setIsDateInputTouched(true);
      default: return null;
    }
  }
  ...
  const isDisabled = !Object.keys(isValid).map(k => isValid[k]).reduce((p,c) => p && c);

  const dateInputStyle = clsx([
    "form-control",
    (isDateInputTouched && isValid.publicacao) && 'is-valid',
    (isDateInputTouched && !isValid.publicacao) && 'is-invalid',
  ])

  const textAreaStyle = clsx([
    "form-control",
    (isTextAreaTouched && isValid.sinopse) && 'is-valid',
    (isTextAreaTouched && !isValid.sinopse) && 'is-invalid',
  ])
      ...
      <div className="border border-muted rounded">
        <form id="create-course-form" className="form needs-validation" noValidate onSubmit={handleClick}>
          ...
          <div className="col-sm-6">
            ...
            <textarea className={textAreaStyle} value={manga.sinopse} placeholder="Escreva aqui a sinopse" rows="5" name="sinopse" required onChange={handleChange} onBlur={() => handleBlur('textarea')}></textarea>
